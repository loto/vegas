 ...
 */
class Change extends ComponentDialog {
  ...
  async promptForNameStep (step) {
    ...
    if (!userProfile.name) {
      ...
      return step.prompt(NAME_PROMPT, 'What is your name?')
    } else {
      return step.next()
    }
  }
  ...
  async promptForCityStep (step) {
    ...
    if (!userProfile.city) {
      return step.prompt(CITY_PROMPT, `Hello ${userProfile.name}, what city do you live in?`)
    } else {
      return step.next()
    }
  }
